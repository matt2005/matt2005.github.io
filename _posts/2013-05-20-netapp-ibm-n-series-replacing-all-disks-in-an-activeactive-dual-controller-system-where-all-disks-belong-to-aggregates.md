...
2. First we turn disk Autoassign off:
on CONTROLLER2:
...
3. We now need to remove the owner from these two disks:
From CONTROLLER2:
...
16. You may now offline and destroy the original root volume and original production aggregate (the one containing the original root volume).
...
17. We can then remove CONTROLLER1 as the owner of the 12 disks that we are replacing.
